...
class AadhaarVerifier {
  ...
  async verifyFile(filePath) {
   ...
   /**
     * Verify from raw XML string (used after extracting XML from ZIP)
     */
    async verifyXml(xmlString) {
    try {
        return await this.verify(xmlString);
    } catch (error) {
        return {
        valid: false,
        verified: false,
        message: `Failed to verify XML: ${error.message}`,
        errors: [error.message]
        };
    }
    }
...
module.exports = AadhaarVerifier;
